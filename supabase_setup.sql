...
-- ===========================================================================
-- LunaVisual v7 migration: site settings, Media rank, creator activity checks
-- Re-running this section is safe.
-- ===========================================================================

-- Public site settings controlled by Admin / Super Admin.
create table if not exists public.site_settings (
  id smallint primary key default 1 check (id=1),
  launcher_url text not null default 'LunaVisual-Launcher.zip',
  ...
  price_7 integer not null default 270 check (price_7>=0),
  price_30 integer not null default 450 check (price_30>=0),
  price_90 integer not null default 670 check (price_90>=0),
  price_lifetime integer not null default 1500 check (price_lifetime>=0),
  alpha_price integer not null default 2400 check (alpha_price>=0),
...
alter table public.site_settings add column if not exists launcher_url text not null default 'LunaVisual-Launcher.zip';
...
create or replace function public.admin_update_lunavisual_site_settings(
  p_launcher_url text,
  p_price_7 integer,
  p_price_30 integer,
  p_price_90 integer,
  p_price_lifetime integer,
  p_alpha_price integer,
...
declare s public.site_settings%rowtype; v_launcher text:=trim(coalesce(p_launcher_url,''));
begin
  if not public.is_lunavisual_admin() then raise exception 'admin_required'; end if;
  if v_launcher='' then raise exception 'launcher_url_required'; end if;
  if not (v_launcher ~* '^https://' or v_launcher ~ '^[A-Za-z0-9._/-]+$') then raise exception 'launcher_url_must_be_https_or_relative'; end if;
  if p_price_7<0 or p_price_30<0 or p_price_90<0 or p_price_lifetime<0 or p_alpha_price<0 then raise exception 'invalid_price'; end if;
  if p_discount_percent<0 or p_discount_percent>90 then raise exception 'invalid_discount'; end if;
  if trim(coalesce(p_support_email,'')) !~* '^[^@[:space:]]+@[^@[:space:]]+\.[^@[:space:]]+$' then raise exception 'invalid_support_email'; end if;
  if trim(coalesce(p_telegram_url,'')) !~* '^https://' then raise exception 'telegram_url_must_be_https'; end if;
  if trim(coalesce(p_discord_url,'')) !~* '^https://' then raise exception 'discord_url_must_be_https'; end if;
  if trim(coalesce(p_funpay_url,'')) !~* '^https://' then raise exception 'funpay_url_must_be_https'; end if;

  insert into public.site_settings(id) values(1) on conflict(id) do nothing;
  update public.site_settings set
    launcher_url=v_launcher,
    price_7=p_price_7,price_30=p_price_30,price_90=p_price_90,price_lifetime=p_price_lifetime,alpha_price=p_alpha_price,
    discount_enabled=coalesce(p_discount_enabled,false),discount_percent=p_discount_percent,discount_label=coalesce(nullif(trim(p_discount_label),''),'Скидка'),discount_until=p_discount_until,
    support_email=trim(p_support_email),telegram_url=trim(p_telegram_url),discord_url=trim(p_discord_url),funpay_url=trim(p_funpay_url),
    updated_by=auth.uid(),updated_at=now()
  where id=1 returning * into s;
  return to_jsonb(s);
end $$;
grant execute on function public.admin_update_lunavisual_site_settings(text,integer,integer,integer,integer,integer,boolean,integer,text,timestamptz,text,text,text,text) to authenticated;

-- Media is a visible profile rank below Admin; it does not grant admin-panel permissions.
create or replace function public.admin_set_lunavisual_rank(p_user_id uuid,p_rank text)
returns boolean language plpgsql security definer set search_path=public as $$
begin
  if not public.is_lunavisual_admin() then raise exception 'admin_required'; end if;
...
-- Creator activity snapshots. Edge Functions update these fields from official APIs.
alter table public.creator_connections add column if not exists recent_video_count_7d integer not null default 0;
alter table public.creator_connections add column if not exists recent_video_count_14d integer not null default 0;
alter table public.creator_connections add column if not exists activity_ok boolean not null default false;
alter table public.creator_connections add column if not exists activity_note text;
alter table public.creator_connections add column if not exists last_content_at timestamptz;

alter table public.creator_applications add column if not exists recent_video_count_7d_at_apply integer not null default 0;
alter table public.creator_applications add column if not exists recent_video_count_14d_at_apply integer not null default 0;
alter table public.creator_applications add column if not exists activity_ok_at_apply boolean not null default false;
...
begin
  ...
  insert into public.creator_applications(
    user_id,provider,requested_rank,connection_id,follower_count_at_apply,quality_score_at_apply,
    recent_video_count_7d_at_apply,recent_video_count_14d_at_apply,activity_ok_at_apply,applicant_note
  ) values(
    auth.uid(),p_provider,v_rank,c.id,c.follower_count,c.quality_score,
    coalesce(c.recent_video_count_7d,0),coalesce(c.recent_video_count_14d,0),coalesce(c.activity_ok,false),nullif(trim(p_note),'')
...
-- Only Admin / Super Admin can make the final decision on creator applications.
create or replace function public.admin_review_creator_application(p_application_id bigint,p_approve boolean,p_note text default null)
returns boolean language plpgsql security definer set search_path=public as $$
declare a public.creator_applications%rowtype;
begin
...
-- Ensure creator rows are visible to their owner and Admin/Super Admin.
drop policy if exists creator_connections_read on public.creator_connections;
create policy creator_connections_read on public.creator_connections for select to authenticated using(user_id=auth.uid() or public.is_lunavisual_admin());
drop policy if exists creator_applications_read on public.creator_applications;
create policy creator_applications_read on public.creator_applications for select to authenticated using(user_id=auth.uid() or public.is_lunavisual_admin());
...
-- ===========================================================================
-- LunaVisual v8 patch: GitHub routes, model download ownership, admin creator refresh UI
-- ===========================================================================
drop policy if exists lunavisual_models_assets_owner_read on storage.objects;
create policy lunavisual_models_assets_owner_read on storage.objects
for select to authenticated
using (
  bucket_id='models-assets' and (
    public.is_lunavisual_admin()
    or exists (
      select 1 from public.models m
      join public.user_models um on um.model_id=m.id
